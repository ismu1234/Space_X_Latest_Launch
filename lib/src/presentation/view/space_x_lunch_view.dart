...
import '../../core/components/refresh_indicator/custom_refresh_indicator.dart';
...
class _SpaceXLunchViewState extends State<SpaceXLunchView> {
    ...
    return BlocBuilder<SpaceXLaunchBloc, SpaceXLaunchState>(
      builder: (context, state) {
        if (state is SpaceXLaunchBlocLoaded) {
          ...
          return _hasDataState(loadList, state);
  ...
  Center _loadingState() {
    return Center(
      child:
          CircularProgressIndicator.adaptive(backgroundColor: ColorTones.red),
    );
  }
